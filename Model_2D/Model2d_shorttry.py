...
Changed
    if if_perturb == False:
        m = r.simulate (0, 10, 10)
    else:
        m1 = r.simulate(0, 10, 10)
"""
...
class Model2d:
    ...
    def plot_all_gts(self, data_all, gtypes, norm_all=True):
        ...
        fig, axs = plt.subplots(7, 6, figsize=[12,7.5])  # plt.subplots(Ver=7, Hor=6, figsize=[8,5])
        #　Original  fig, axs = plt.subplots(ncols=len(data_all[0]), nrows=n_tpts, figsize=[8,5])

        fig.subplots_adjust(wspace=0.01, hspace=0.01, bottom=0.05, top=0.95) #margin
        cax = fig.add_axes([0.93,0.7,0.01,0.21])  #fig.add_axes([x,y,width,height])
            ...
            for j in range(axs.shape[1]):
                ...
                for ci, poly in enumerate(self.polys):
                    ...
                    f = ax.fill(*zip(*poly), alpha=0.99, edgecolor='k', 
                            facecolor=cmap(scaled_level))  # fill polygon
